...
startBtn.addEventListener('click', async () => {
  try {
    ...
    if (!apiKey || !model || !file || !numQuestions || numQuestions < 1) {
    ...
    await generateAndRenderQuiz({ apiKey, model, sourceText: lastSourceText, count: targetQuestionCount, round });
  } catch (error) {
    console.error(error);
...
function renderResults({ details, correctCount, wrongCount, total }) {
  resultSection.classList.remove('hidden');
  const lines = details.map((d, i) => {
    const className = d.isCorrect ? 'correct' : 'incorrect';
    const yourAnswer = d.selectedIndex >= 0 ? d.options[d.selectedIndex] : 'No contestada';
    return `
      <li class="${className}">
        <strong>${i + 1}. ${escapeHtml(d.question)}</strong><br/>
        La teva resposta: ${escapeHtml(yourAnswer)}<br/>
        Correcta: ${escapeHtml(d.options[d.correctIndex])}
      </li>
    `;
  }).join('');
  ...
  resultSection.innerHTML = `
    <h2>Resultats</h2>
    <p><strong>${correctCount}/${total}</strong> correctes.</p>
    <p>${wrongCount === 0 ? 'Perfecte! No cal repetir.' : 'Com a mínim una malament: es pot generar un nou qüestionari del mateix tamany.'}</p>
    <ol>${lines}</ol>
  `;
}
...
Text font:\n${sourceText.slice(0, 120000)}`;
  ...
  try {
    ...
    throw new Error(`JSON invàlid rebut: ${text}`);
  }

  if (!Array.isArray(parsed.questions) || parsed.questions.length !== count) {
    throw new Error('El model no ha retornat el nombre correcte de preguntes.');
  }

  return parsed.questions.map((q, i) => {
    if (!q.question || !Array.isArray(q.options) || q.options.length !== 4 || !Number.isInteger(q.correctIndex)) {
      throw new Error(`Pregunta invàlida a l'índex ${i}`);
    }
    return q;
  });
}
